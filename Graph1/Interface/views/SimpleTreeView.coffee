...
class SimpleTreeView extends View
    constructor: ( @el, @model) ->      # Prend comme paramètres un élément de page et un modèle de type GraphData
        super @model
    ...
    onchange: ->
        if @model.has_been_modified   
            ...
            data = []                   # Initialisation    
            @tojson data, @model        # Conversion du modèle en liste Json
            
            root = data[0]              # Initialisation
            @update_tree root           # Création de l'arbre
